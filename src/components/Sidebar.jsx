...
export default function Sidebar({ isOpen, onToggle }) {
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="flex items-center px-3 py-4 border-b" style={{ borderColor: 'var(--border)' }}>
        ...
        {!collapsed && (
          <>
            <span className="text-sm font-semibold tracking-tight ml-2 flex-1 whitespace-nowrap overflow-hidden"
              style={{ color: 'var(--text-primary)' }}>
              DevPortal
            </span>
            <button onClick={toggle} className="p-1 rounded hover:bg-white/5 transition-colors ml-1 flex-shrink-0">
              <ChevronLeft />
            </button>
          </>
        )}
      ...
      <nav className="flex-1 overflow-y-auto py-3 px-2 space-y-0.5">
        ...
        {/* Apps section header + sort control */}
        {!collapsed && (
...
function NavItem({ to, icon, label, color = 'text-cyan-400', status, collapsed, exact }) {
  return (
    <NavLink to={to} end={exact}>
      {({ isActive }) => (
        <div className={`flex items-center gap-2.5 px-2 py-2 rounded-md text-sm transition-all duration-150 group relative cursor-pointer
